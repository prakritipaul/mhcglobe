...
BERT_output_dir = "/home/ppaul/mhcglobe_test_files/BERT_colab_run_outputs_24_05_01/"
...
training_allele_pseudoseqs = training_allele_pseudoseqs.values.tolist()
training_peptide_pseudoseqs = training_peptide_pseudoseqs.values.tolist()
es_allele_pseudoseqs = es_allele_pseudoseqs.values.tolist()
es_peptide_pseudoseqs = es_peptide_pseudoseqs.values.tolist()

def get_seqs(list_of_pseudoseqs):
    return [sublist[0] for sublist in list_of_pseudoseqs]

training_allele_pseudoseqs = get_seqs(training_allele_pseudoseqs)
training_peptide_pseudoseqs = get_seqs(training_peptide_pseudoseqs)
es_allele_pseudoseqs = get_seqs(es_allele_pseudoseqs)
es_peptide_pseudoseqs = get_seqs(es_peptide_pseudoseqs)
...
print("Getting BERT embeddings for first 5")
# Get BERT embeddings
training_allele_BERT_embeddings = phf.get_BERT_embeddings(training_allele_pseudoseqs[:5], tokenizer, model, "tf")
training_peptide_BERT_embeddings = phf.get_BERT_embeddings(training_peptide_pseudoseqs[:5], tokenizer, model, "tf")
